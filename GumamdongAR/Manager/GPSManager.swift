...
class GPSManager: NSObject, ObservableObject, CLLocationManagerDelegate {
    ...
    func getDisitance() -> CLLocationDistance {
        if let targetLocation = targetCoordinate, let currentLocation = coordinate {
           let currentCLLocation = CLLocation(latitude: currentLocation.latitude, longitude: currentLocation.longitude)
           let targetCLLocation = CLLocation(latitude: targetLocation.latitude, longitude: targetLocation.longitude)
            print("거리는 \(targetCLLocation.distance(from: currentCLLocation))")
            return targetCLLocation.distance(from: currentCLLocation)
        } else {
            return 100
        }
    }
